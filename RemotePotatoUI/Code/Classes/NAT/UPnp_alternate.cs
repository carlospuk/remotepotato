...
namespace RemotePotatoServer
{
    public class UPnPLowLevel
    {
        ...
        public static void OpenFirewallPort(int port)
        {
            ...
            foreach (System.Net.NetworkInformation.NetworkInterface nic in nics)
            {
                try
                {
                    ...
                    Debug.Print("NIC found, Machine IP: " + machineIP);
                    ...
                    foreach (System.Net.NetworkInformation.GatewayIPAddressInformation gwInfo in nic.GetIPProperties().GatewayAddresses)
                    {
                        try
                        {
                            Debug.Print("Gateway: " + gwInfo.Address.ToString());
        ...
        private static void openPortFromService(string services, string serviceType, string machineIP, string firewallIP, int gatewayPort, int portToForward)
        {
            ...
            "<NewPortMappingDescription>RemotePotatoAutoFwd</NewPortMappingDescription>" +
            ...
            string url = CreateURL(firewallIP, gatewayPort.ToString(), controlUrl);
        ...
        static string CreateURL(string firewallIP, string gatewayPort, string controlURL)
        {
            // Is Control URL an absolute URL?
            if (
                (controlURL.Contains(":")) || (controlURL.Contains("."))
                )
            {
                return controlURL;
            }
            ...
            {
                return "http://" + firewallIP + ":" + gatewayPort + controlURL;
            }
        }
