...
namespace FatAttitude.WTVTranscoder
{
    ...
    public sealed class FilterGraphTools
    {
        ...
        public static List<IBaseFilter> AddPreferredFilters(IGraphBuilder graph, string PreferredFilters)
        {
            ...
            foreach (string filtName in PreferredFilters.Split(new char[] { ';', ',' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                try
                {
                    if (Regex.IsMatch(filtName, @"{?\w{8}-\w{4}-\w{4}-\w{4}-\w{12}}?"))
                        filter = FilterGraphTools.AddFilterFromClsid(graph, new Guid(filtName), filtName);
                    else
